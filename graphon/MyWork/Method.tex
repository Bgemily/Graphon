...
\noindent [Write down the loss function for aligning cdf?]
...
\While{not converge}{
  	Update $\{\hat f_{i,q}^{[s+1]}\}_{i\in[n],q\in[k]}$ and $\{\hat f_{l,q}^{[s+1]}\}_{l,q\in[k]}$ with $\mathbf{z}=\mathbf{z}^{[s]}$ and $\tau=\tau^{[0]}$ \;
  	Compute $\{\alpha_{i,q}\}_{i\in[n],q\in[k]}$ with $\mathbf{z}=\mathbf{z}^{[s]}$\;
  	Update $\mathbf{z}^{[s+1]}$ with $\{\hat f_{i,q}^{[s+1]}\}_{i\in[n],q\in[k]}$, $\{\hat f_{l,q}^{[s+1]}\}_{l,q\in[k]}$ and $\{\alpha_{i,q}\}_{i\in[n],q\in[k]}$\;
  	Evaluate the stopping criterion\;
  	$s\leftarrow s+1 $\;
 }
 Update $\tau^{[1]}$ with $\mathbf{z^{[s]}}$ and $\{\hat f_{i,q}^{[s]}\}_{i\in[n],q\in[k]}$\;
\KwOut{$\mathbf{z^{[s]}}$, $\{\hat f_{l,q}^{[s]}\}_{l,q\in[k]}$.}

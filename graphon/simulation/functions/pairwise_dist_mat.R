...
pairwise_dist_mat = function(pdf_array, degree_mat, t_unit = 0.05){
  ...
  for (i in 1:(N_node-1)) {
    for (j in ((i+1):N_node)) {
      ...
      res = get_dist_betw_pdfarray(pdf_array[i, , , drop=F], pdf_array[j, , , drop=F], symmetric=FALSE, weights=weights, t_unit = t_unit)

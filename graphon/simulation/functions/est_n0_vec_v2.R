                         ...
                         max_iter=5, epsilon=0.001)
{
  ...
  if (is.null(n0_vec_list)) {
    n0_vec_list = list()
    for (m in 1:N_subj) {
      res = get_init_v2(edge_time_mat = edge_time_mat_list[[m]], 
                            N_clus = N_clus, t_vec = t_vec)
      n0_vec_list[[m]] = res$n0_vec
    }
  ...
  while(!converge && n_iter<= max_iter)
  {
    ...
    for (m in 1:N_subj) {
      ...
      for (q in 1:N_clus) {
        weight_mat[,q] = rowSums(order_mat_current[,clusters_tmp[[q]]])
      }
      ...
      for (q in 1:N_clus) {
        for (i in clusters_tmp[[q]]) {
          ...
          # f_origin_list = lapply(1:length(clusters), function(l) f_origin_list[[l]]*
    ...
    # plot(truth, n0_vec_list_update[[1]])

...
init_kmeans = function(f_list, k, seed, step_size=0.01)
{
  index = c()
  set.seed(seed); seed = seed+10; index = c(index,sample(length(f_list), 1))
  f_center_list = f_list[index]
  for (i in 2:k) {
    r = re_cluster_gd(f_list, f_center_list, rep(0,length(f_list)), step_size)
...
re_cluster_gd = function(f_list, f_center_list, n0_vec, step_size)
{
  z = vector(mode = "numeric", length = length(f_list))
  clusters = vector(mode = "list", length = length(f_center_list))
  ...
  for (i in (1:length(f_list))) {
    ...
    for (l in (1:length(f_center_list))) {
      ...
      r = align_curves_gd(f, f_center, n0, step_size)
      d = r$dist_min
      if (d < dist_min){
        dist_min = d
        z[i] = l
...
re_center_gd = function(f_list, clusters, n0_vec, step_size, MaxIter=100)
{
  f_center_list = vector(mode = "list", length = length(clusters))
  dist_vec = vector("numeric", length(clusters))
  for (k in (1:length(clusters))) {
    ...
    r = re_center_gd_(f_clusterk_list, n0_clusterk_vec, step_size, MaxIter)
    f_center_list[[k]] = r$f_center
    dist_vec[k] = r$dist
...
re_center_gd_ = function(f_clusterk_list, n0_clusterk_vec, step_size, MaxIter = 100)
{
  ...
  while (dist_reduc>0.01 && iter_count <= MaxIter) {
    ...
    for (i in (1:length(f_clusterk_list))) { # align all f to the current f_center
      r = align_curves_gd(f_clusterk_list[[i]], f_center, n0=n0_clusterk_vec[i], step_size)
    ...
    f_center = mean_curve(f_clusterk_list, n0_clusterk_vec)
  }
  # print(iter_count)
  # plot(f_center, type = 'l')

...
# Implement and Test the algorithm adapted from Bigot and Gendre (2013)
...
est_n0_vec_2 = function(edge_time_mat_list, clusters_list, t_vec=seq(0,50,0.05), 
...
###### Generate nodes, clusters, and edge time matrix ######

total_time = 200
t_vec = seq(0, total_time, length.out = 1001)
t_unit = t_vec[2]-t_vec[1]
SEED = 831
clus_size = 20

set.seed(SEED)
network = generate_network2(total_time=200, tau_max=c(10,10,10)*4, 
                            conn_prob=0.3, beta=1.2, alpha=0.1,
...
### Choose only the first cluster
# edge_time_mat_list = list(edge_time_mat[1:clus_size,1:clus_size])
# clusters_list = list(mem2clus(network$membership_true)[1])
# n0_vec_list_true = list(network$tau_vec[1:clus_size]/t_unit)
# n0_vec_list_true = lapply(n0_vec_list_true, function(vec) vec-min(vec))

edge_time_mat_list = list(edge_time_mat)
clusters_list = list(mem2clus(network$membership_true))
n0_vec_list_true = list(network$tau_vec/t_unit)
n0_vec_list_true = lapply(n0_vec_list_true, function(vec) vec-min(vec))
